...
import MyOrderListGlobalFilter from './OrdersFilter';
...
describe('MyOrderListGlobalFilter', () => {
  const defaultProps = {
    updateFilters: vi.fn(),
    setValue: vi.fn(),
    filters: {},
  ...
  const renderComponent = (props = {}) => {
    return render(
      <BrowserRouter>
        <MyOrderListGlobalFilter {...defaultProps} {...props} />
      </BrowserRouter>
    );
  };
  ...
  it('clears all filters when reset button is clicked', () => {
    renderComponent({
      filters: { orderId: 'test', orderStatus: 'PENDING' },
      value: 'test',
    });
  ...
  it('disables apply button when no filters are selected', () => {
    renderComponent();
    const filterButton = screen.getByRole('button', { name: /filter/i });
    fireEvent.click(filterButton);
